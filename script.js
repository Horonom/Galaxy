...
let about_item = document.querySelectorAll('.merch-item-descr')
let about = ['Тарілка з принтом Марса. Доступна в червоному та білому кольорах','Кружка з принтом Марса. Доступна в червоному та білому кольорах','Масштабна модель однієї з ракет Galaxy','Масштабна модель марсохода (ми купуємо їх у NASA)']
for (let y = 0; y <4; y +=1) {
    about_item[y].innerHTML = about[y]
}
let item_price = document.querySelectorAll('.merch-item-price')
let price = ['1000', '1000', '2500', '5000']
for (let j = 0; j <4; j +=1) {
    item_price[j].innerHTML = price[j]
}
let buttonCartsAmount = document.querySelectorAll('.add-to-cart-btn')

for (let u = 0; u<buttonCartsAmount.length; u++) {
    buttonCartsAmount[u].addEventListener('click', function() {
        const oldValue = +document.querySelector('.cart-amount').innerHTML;
        document.querySelector('.cart-amount').innerHTML = oldValue + 1;
